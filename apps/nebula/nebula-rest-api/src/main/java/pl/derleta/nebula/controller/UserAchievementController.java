...
public final class UserAchievementController {
     ...
     * @param jwtToken      the JWT token extracted from the "jwtToken" cookie used for authentication and validation
    ...
    public ResponseEntity<UserAchievementResponse> get(@PathVariable Long userId, @PathVariable Integer achievementId,
        ...
        if (tokenProvider.isValid(jwtToken)) {
            if (Objects.equals(userId, tokenProvider.getUserId(jwtToken))) {
     ...
     *
     * @param jwtToken the JWT token extracted from the cookie to authenticate and identify the user
    ...
    public ResponseEntity<List<UserAchievementResponse>> getList(@CookieValue("jwtToken") String jwtToken) {
        if (tokenProvider.isValid(jwtToken)) {
     ...
     * @param sortOrder  the order in which the results should be sorted, default is "asc" (ascending order).
     ...
     * @param jwtToken   the JWT token extracted from the cookie header, used for authentication/authorization.
