...
public final class GameController {
     ...
     *
     * @param jwtToken The JWT token extracted from the client's cookies for authentication and authorization purposes.
    ...
    public ResponseEntity<GameResponse> add(@CookieValue("jwtToken") String jwtToken, @RequestBody GameNewRequest request) {
        if (authorizationService.notContainsAdminRole(jwtToken))
     ...
     *
     * @param jwtToken the JWT token extracted from the cookie to validate the user's authentication and authorization
    ...
    public ResponseEntity<GameResponse> update(@CookieValue("jwtToken") String jwtToken,
     ...
     *
     * @param jwtToken the JWT token extracted from the request's cookies for authentication
     * @param id       the ID of the resource to be deleted
     * @return a ResponseEntity containing the HTTP status code and optional body
     */
    @DeleteMapping("/" + DEFAULT_PATH + "/{id}")
    public ResponseEntity<String> delete(@CookieValue("jwtToken") String jwtToken, @PathVariable Integer id) {
        if (authorizationService.notContainsAdminRole(jwtToken))
            return ResponseEntity.status(HttpStatus.FORBIDDEN).body(null);
