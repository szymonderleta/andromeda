...
public class RoleService {
     ...
     */
    public Set<Role> getListUsingSQLFilter(final String roleNameFilter) {
     ...
     */
    public Page<Role> getPageUsingSQLFilter(final int page, final int size, final String sortBy, final String sortOrder, final String roleNameFilter) {
    ...
    /**
     * Retrieves a Role object based on the provided role ID.
     *
     * @param roleId the unique identifier of the role to be retrieved
     * @return the Role object corresponding to the specified role ID
     */
    public Role get(final int roleId) {
        return RoleApiMapper.toRole(
                repository.findById(roleId)
        );
    }

    /**
     * Saves a new role in the repository. The method assigns a unique role ID to the role,
     * persists the role in the repository, and returns the newly created role.
     *
     * @param role the role object containing the role details to be saved
     * @return the saved Role object with the assigned ID
     */
    public Role save(Role role) {
        int roleId = repository.getNextRoleId();
        repository.save(roleId, role);
