...
public class AccountsServiceImpl implements AccountsService {
    ...
    public AccountResponse register(final UserRegistrationRequest request) {
        ...
        AccountProcess accountProcess = new AccountProcessFactoryImpl().create(AccountProcessType.USER_REGISTRATION, repositories, emailService);
    ...
    /**
     * Retrieves the user roles for the specified username and email.
     *
     * @param username the username associated with the user roles
     * @param email    the email associated with the user roles
     * @return the UserRoles object corresponding to the specified username and email
     */
    @Override
    public UserRoles get(final String username, final String email) {
        return UserRolesApiMapper.toUserRoles(
                userRolesRepository.get(username, email)
        );
    }
    ...
    public AccountResponse confirm(final UserConfirmationRequest request) {
        final Set<RepositoryClass> repositories = new HashSet<>(Set.of(userRepository, userRoleRepository, confirmationTokenRepository));
        AccountProcess accountProcess = new AccountProcessFactoryImpl().create(AccountProcessType.CONFIRMATION_TOKEN, repositories, emailService);
        AccountResponse status = accountProcess.check(request);
        if (!status.isSuccess()) return status;
    ...
    public AccountResponse unlock(final UserUnlockRequest request) {
        final Set<RepositoryClass> repositories = new HashSet<>(Set.of(userRepository, userRoleRepository, confirmationTokenRepository));
        AccountProcess accountProcess = new AccountProcessFactoryImpl().create(AccountProcessType.UNLOCK_ACCOUNT, repositories, emailService);
        AccountResponse status = accountProcess.check(request);
        if (!status.isSuccess()) return status;
        if (accountProcess instanceof CreateConfirmationProcess instance) {
            UserEntity userEntity = accountProcess.save(request);
    ...
    public AccountResponse resetPassword(final ResetPasswordRequest request) {
        final Set<RepositoryClass> repositories = new HashSet<>(Set.of(userRepository, userRoleRepository, confirmationTokenRepository));
        AccountProcess accountProcess = new AccountProcessFactoryImpl().create(AccountProcessType.RESET_PASSWORD, repositories, emailService);
        AccountResponse status = accountProcess.check(request);
        if (!status.isSuccess()) return status;
    ...
    public AccountResponse updatePassword(final ChangePasswordRequest request) {
        ...
        AccountProcess accountProcess = new AccountProcessFactoryImpl().create(AccountProcessType.CHANGE_PASSWORD, repositories, emailService);
        AccountResponse status = accountProcess.check(request);
        if (!status.isSuccess()) return status;
