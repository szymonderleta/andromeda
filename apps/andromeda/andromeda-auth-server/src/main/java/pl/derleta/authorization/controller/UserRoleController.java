...
public class UserRoleController {
    ...
    /**
     * Retrieves the user role details for the specified ID.
     * Accessible by users with "ROLE_USER", "ROLE_TESTER", "ROLE_MODERATOR", or "ROLE_ADMIN" roles.
     *
     * @param id the ID of the user role to retrieve
     * @return a {@link ResponseEntity} containing the {@link UserRoleResponse} and a status code of 200 (OK)
     */
    @RolesAllowed({"ROLE_USER", "ROLE_TESTER", "ROLE_MODERATOR", "ROLE_ADMIN"})
    ...
    public ResponseEntity<UserRoleResponse> get(@PathVariable Integer id) {
        UserRoleResponse response = userRoleModelAssembler.toModel(
                service.get(id)
        );
